...
/*   Updated: 2023/11/14 16:13:53 by carmarqu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	put_img(t_data *data, char c, int x, int i)
{
	...
	if (c == 'H')
		mlx_image_to_window(data->mlx_ptr, data->img->hunter_r, x * 32, i * 32);
	return (1);
}
...
int	create_img(t_data *data)
{
	...
	data->img->hunter_r = mlx_texture_to_image(data->mlx_ptr,
			data->text->hunter_r);
	...
	mlx_delete_texture(data->text->hunter_r);
	img_to_win(data);
	return (1);
}
